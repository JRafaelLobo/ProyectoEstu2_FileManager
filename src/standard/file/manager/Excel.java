...
public class Excel extends Archivos {
    ...
    public boolean llenarTabla(ArrayList<Campo> campos) {
...
//        xlsFirstTable.easy_getCell(0, 0).setDataType(DataType.STRING);
        ...
        try (RandomAccessFile file = new RandomAccessFile(this.rutaArchivo, "rw")) {
            ...
            while (i < file.length()) {
                ...
                for (int j = 0; j < register.length; j++) {
                    xlsFirstTable.easy_getCell(count, j).setValue(register[j]);
                    xlsFirstTable.easy_getCell(count, j).setDataType(DataType.STRING);
                }
    ...
    public boolean exportar(String ruta, Archivos file) {
        this.rutaArchivo = file.getRutaArchivo();
        this.longitudTotalDeMetadata = file.longitudTotalDeMetadata;
        this.longitudTotalRegistro = file.longitudTotalRegistro;
        
        llenarTabla(campos);
        WB.easy_WriteXLSXFile(ruta + ".xlsx");
